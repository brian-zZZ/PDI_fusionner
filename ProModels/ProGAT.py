...
class ProGAT(nn.Module):
    ...
    def forward(self, amino_list, amino_degree_list, amino_mask):
        ...
        # batch_size, seq_len = amino_list.shape # brian
        ...
        for d in range(self.radius - 1):
            ...
            # (batch_size, max_seq_len, max_neighbor_num, embedding_dim*2)
            feature_align = torch.cat([amino_feature_expand, neighbor_feature], dim=-1)

            # (batch_size, max_seq_len, max_neighbor_num, 1)
            align_score = F.leaky_relu(self.align[d + 1](self.dropout(feature_align)))
            
            align_score = align_score + softmax_mask # (batch_size, max_seq_len, max_neighbor_num, 1)
            attention_weight = F.softmax(align_score, -2)
            
            attention_weight = attention_weight * attend_mask # (batch_size, max_seq_len, max_neighbor_num, 1)
            
            # (batch_size, max_seq_len, max_neighbor_num, num_amino_feature)
            neighbor_feature_transform = self.attend[d + 1](self.dropout(neighbor_feature))
            
            # (batch_size, max_seq_len, num_amino_feature)
            context = torch.sum(torch.mul(attention_weight, neighbor_feature_transform), -2)
            ...
            # do nonlinearity, (batch_size, max_seq_len, num_amino_feature)
            activated_features = F.relu(amino_feature)
        ...
        for t in range(self.T):
            ...
            # (batch_size, max_seq_len, num_amino_feature*2)
            ...
            # (batch_size, max_seq_len, num_amino_feature) -> (batch_size, num_amino_feature)
